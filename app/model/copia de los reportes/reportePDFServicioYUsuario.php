...
//Reporte Turnos
//echo "select count(*) from auditoria where Estado != 'NORMAL' and auditoria.FechaLlegada  and IdUsuario = $idusuario and auditoria.FechaLlegada >= '$fechaInicial 00:00:00' and auditoria.FechaLlegada<='$fechafinal 23:59:59'";
//exit();
$TurnosCount = DevolverUnDato("select count(*) from auditoria where Estado != 'NORMAL' and auditoria.FechaLlegada  and IdUsuario = $idusuario and auditoria.IdServicio = $idServicio and auditoria.FechaLlegada >= '$fechaInicial 00:00:00' and auditoria.FechaLlegada<='$fechafinal 23:59:59'");
$TurnosAtendidosCount = DevolverUnDato("select count(*) from auditoria where Estado = 'TERMINADO' and Observacion = '' and IdUsuario = $idusuario and auditoria.IdServicio = $idServicio and auditoria.FechaLlegada >= '$fechaInicial 00:00:00' and auditoria.FechaLlegada<='$fechafinal 23:59:59'");
$TurnosAusentesCount = DevolverUnDato("select count(*) from auditoria where Estado = 'AUSENTE' and IdUsuario = $idusuario and auditoria.IdServicio = $idServicio and auditoria.FechaLlegada >= '$fechaInicial 00:00:00' and auditoria.FechaLlegada<='$fechafinal 23:59:59'");
$TurnosTransferidosCount = DevolverUnDato("select count(*) from auditoria where Estado = 'TERMINADO' and Observacion = 'TRANSFERIDO' and IdUsuario = $idusuario and auditoria.IdServicio = $idServicio and auditoria.FechaLlegada >= '$fechaInicial 00:00:00' and auditoria.FechaLlegada<='$fechafinal 23:59:59'");


//servicios
$servicios = DevolverUnArreglo("select servicio.Servicio, COUNT(auditoria.IdServicio) as Cantidad from auditoria JOIN servicio on (auditoria.IdServicio = servicio.IdServicio) where (Estado != 'NORMAL') and auditoria.IdUsuario = $idusuario and auditoria.IdServicio = $idServicio and auditoria.FechaLlegada >= '$fechaInicial 00:00:00' and auditoria.FechaLlegada<='$fechafinal 23:59:59' GROUP by auditoria.IdServicio");
...
//exit();
$detalle = DevolverUnArreglo("select auditoria.* , servicio.Servicio,usuario.NombreCompleto from auditoria,servicio,usuario where (auditoria.IdServicio = servicio.IdServicio) and (auditoria.IdUsuario = usuario.IdUsuario) and (auditoria.Estado = 'TERMINADO' or auditoria.Estado = 'AUSENTE') and auditoria.IdUsuario = $idusuario and auditoria.IdServicio = $idServicio and auditoria.FechaLlegada >= '$fechaInicial 00:00:00' and auditoria.FechaLlegada<='$fechafinal 23:59:59'");
//datos del usuario o servicio 
$NombreUsuario1 = DevolverUnDato("select NombreCompleto from usuario where idusuario =$idusuario ");
$nombreServicio1 = DevolverUnDato("select servicio from servicio where idservicio =$idServicio");
    ...
    <body class="cuerpo">
        ...
        <DIV class="BORDESOTROS">
            <div style="margin-top: 15px;">
                ...
                <div>Usuario: <?php echo $NombreUsuario1 ?></div>
                <div> Servicio <?php echo $nombreServicio1 ?></div>
            </div>
            <div class="divfechas">
